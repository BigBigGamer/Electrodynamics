...
% \pgfplotstableread{data.tsv}\mytable
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			%
			xlabel={$\,\,\varkappa_1 l$}, 			% ПодПись оси Y
			ylabel={$pl$}, 			% ПодПись оси X
			%
			major grid style={
		...
		% \addplot[dashed, blue!50,domain={-3*pi/:3*pi/2},samples=1000] {tan(deg(x))};

		\addplot[black, domain={0:5*pi/2},samples=1000] {x*tan(deg(x))};
		...
		\xdef\w{(pi/2+pi/4)}
		\addplot[red, domain={0:\w},samples=1000] {2.5*sqrt(\w^2-x^2)};
...
%		\draw[fill=black,dashed] (\HH,\XX) circle (0.5pt) -- (,\XX) circle (0.5pt);
